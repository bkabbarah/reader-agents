...
sh ocr/run_nougat_batch0.sh
echo "=== $(date -u +%H:%M:%SZ) followups: K&S truncated pages at 4096 tokens ===" >> "$LOG"
$PY ocr/nougat_ocr.py corpus/batch0/brownian-motion-and-stochastic-calculus-karatzas-shreve-2ed.pdf data/extracted/batch0/karatzas-shreve/nougat \
...
$PY ocr/nougat_ocr.py corpus/pdf/trench-ra.pdf data/extracted/trench-ra/nougat --batch 8 2>&1 \
   | grep --line-buffered -v -i "warning\|Loading weights\|still open" >> "$LOG"
